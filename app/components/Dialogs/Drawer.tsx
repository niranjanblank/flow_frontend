...
const Drawer = ({ children, drawerComponent }: { children: React.ReactNode, drawerComponent: React.ReactNode }) => {
  ...
  return (
    <>
      ...
      {/* Drawer */}
      <div
        className={`fixed right-0 top-0 w-64 h-full bg-zinc-800 text-gray-400 shadow-lg p-4 z-50 transition-transform transform ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}
      >
